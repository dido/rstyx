...
class MessageTest < Test::Unit::TestCase
  ...
  def test_qid
    ...
    assert_equal("ASCII-8BIT", bytes.encoding.name)
  ...
  def test_rerror
    ...
    bytes = rerr.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x0e\x00\x00\x00\x6b\x34\x12\x05\x00error".force_encoding("ASCII-8BIT")
    assert(bytes == expect)
  ...
  def test_tflush
    ...
    bytes = tf.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x6c\x34\x12\x78\x56".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
  ...
  def test_rflush
    ...
    bytes = tf.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x6d\x34\x12".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
  ...
  def test_topen
    ...
    bytes = to.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x70\x34\x12\x78\x56\x34\x12\x51".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
  ...
  def test_rwrite
    ...
    bytes = rw.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x77\x34\x12\x78\x56\x34\x12".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
  ...
  def test_tclunk
    ...
    packlen = [len].pack("V")
    expect = packlen + expect
    assert_equal("ASCII-8BIT", tc.to_bytes.encoding.name)
  ...
  def test_rclunk
    ...
    bytes = rc.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x79\x34\x12".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
  ...
  def test_rremove
    ...
    bytes = rr.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x7b\x34\x12".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
  ...
  def test_rwstat
    ...
    bytes = rw.to_bytes
    assert_equal("ASCII-8BIT", bytes.encoding.name)
    expect = "\x7f\x34\x12".force_encoding("ASCII-8BIT")
    len = expect.length + 4
    packlen = [len].pack("V")
    expect = packlen + expect
    assert(expect == bytes)
